var respuestas = [{ name: 'TRANSICIÓN' }, { name: 'EMPRESAS' }, { name: 'INTERVENTOR' }, { name: 'INGRESO' }, { name: 'SINDICAL'}, {name:"PRODUCCIÓN"}, {name:"INTERNACIONAL"}, {name:"CLÁSICO"}];
(function ($) {
    Sopa = function ($el, options) {
        $t = $("<table border='1'>");

        var defaults = { palabras:respuestas, complejo: 20, vertical: 'S', onWin: "" };
        var aciertos = 0;
            ...
            if (cantidadclicks == 1) {
                ...
                $.each(defaults.palabras, function () {
                    if (selecion == this.name) {
                        ...
                        retroalimentar("Encontraste la palabra: <b style='color:#6699CC;'>" + selecion+"</b>");
						for(var i = 0; i<respuestas.length; i++){
							if(respuestas[i].name == selecion){
								$('span#resp'+i).html("<b style='color:#6699CC;'>"+selecion.toLowerCase()+"</b>");
							}
						}
                        ...
                        if (aciertos == defaults.palabras.length) {
                            retroalimentar("<b style='color:#6699CC;'>Felicitaciones!!!. Has encontrado todas las palabras.</b");
                            $g.onWin();
                        }
                    }
                });
...
function nogano(aciertos){
	console.log("nogano aciertos: "+aciertos);
}
function noganopalabras(miradorpalabras){
	console.log("noganopalabras miradorpalabras: "+miradorpalabras);
}
function mostrarTodo(informeFinal){
	console.log("Terminaste. Informe final: "+informeFinal);	
}